...
class ExceedListModal extends React.Component {
	...
	componentDidMount() {
		let { curKey, record, operation } = this.props;
			...
			if (!err) {
				...
				if (curKey === 'new') {
					postCreatePayment(values).then(() => {
						this.props.onCancel();
						getPaymentList({ ...filterParams, page: 1, page_size, operation: 1 });
						hide();
				...
				postUpdatePayment({ ...values, id: record.id }).then(() => {
					this.props.onCancel();
					getPaymentList({ ...filterParams, page, page_size, operation: 1 });
					hide();
					message.success('操作成功！');
					this.setState({ isClick: false });
				}).catch(({ errorMsg }) => {
	...
	render() {
		...
		>
			<Form className='new-modal-wrap'>
				...
				<Row type="flex" justify="start">
					<FormItem className='wrap-control' label='打折比例' {...formItemLayout}>
