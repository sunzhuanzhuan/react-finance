...
//打款单列表
export const getPaymentSlipList = (obj) => dispath => {
	return api.get('/flash/payment_slip_list', { params: obj }).then(response => {
		const { data } = response;
		dispath({ type: Types.GET_PAYMENT_SLIP_LIST, payload: data });
	})
}
//新建打款单
export const getPaymentSlipOrderList = (obj) => dispath => {
	return api.get('/flash/payment_slip_order_list', { params: obj }).then(response => {
		const { data } = response;
		dispath({ type: Types.GET_PAYMENT_SLIP_ORDER_LIST, payload: data });
	})
}
//打款单详情
export const getPaymentSlipDetail = (obj) => dispath => {
	return api.get('/flash/payment_slip_detail', { params: obj }).then(response => {
		const { data } = response;
		dispath({ type: Types.GET_PAYMENT_SLIP_DETAIL, payload: data });
	})
}
//打款单详情excel
export const detailForExcel = (obj) => dispath => {
	return api.get('/flash/detail_for_excel', { params: obj }).then(response => {
		const { data } = response;
		dispath({ type: Types.GET_DETAIL_FOR_EXCEL, payload: data });
	})
}
//导出打款单接口
export const excelNameList = (obj) => dispath => {
	return api.get('/flash/excel_name_list', { params: obj }).then(response => {
		const { data } = response;
		dispath({ type: Types.EXCEL_NAME_LIST, payload: data });
	})
}
//提交新建打款单
export const addPaymentSlip = (obj) => () => {
	return api.get('/flash/add_payment_slip', { params: obj }).then(response => {
		return response
	}).catch(err => {
		return err;
	})
...
//打款单确认
export const paymentSlipAudit = (obj) => () => {
	return api.get('/flash/payment_slip_audit', { params: obj }).then(response => {
		return response
	}).catch(err => {
		return err;
	})
...
//下载excel
export const downExcel = (obj) => () => {
	return api.get('/flash/down_excel', { params: obj }).then(response => {
...
//已打款
export const paymentSlipPayed = (obj) => () => {
	return api.get('/flash/payment_slip_payed', { params: obj }).then(response => {
		return response
	}).catch(err => {
		return err;
	})
...
//已还款
export const paymentSlipReturned = (obj) => () => {
	return api.get('/flash/payment_slip_returned', { params: obj }).then(response => {
		return response
	}).catch(err => {
		return err;
	})
...
//已节税
export const paymentSlipPayedTax = (obj) => () => {
	return api.get('/flash/payment_slip_payed_tax', { params: obj }).then(response => {
		return response
	}).catch(err => {
		return err;
	})
...
export const getBillList = (obj) => dispath => {
	return api.get('/flash/bill_list', { params: obj }).then(response => {
		const { data } = response;
		dispath({ type: Types.GET_BILL_LIST, payload: data, status_type: obj.status_type });
	}).catch(err => {
		return err;
	})
}
