...
export const getAssociatedOrders = (id, product_line, page, page_size) => dispatch => {
	return api.get(`/finance/invoice/application/associatedOrders?id=${id}&product_line=${product_line}&page=${page}&page_size=${page_size}`).then(response => {
		const { data } = response;
